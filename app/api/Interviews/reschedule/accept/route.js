...
export async function POST(request) {
    try {
        ...
        await interviewRef.update({
            ...
            approved_reschedule_at: FieldValue.serverTimestamp()
        });
        ...
        await adminDB.collection("notifications").add({
            ...
            title: "Interview Reschedule Approved",
            message: `Your interview has been rescheduled to ${interview.scheduled_at.toDate()}`,
            created_at: FieldValue.serverTimestamp()
        });

        return NextResponse.json({ message: "Reschedule approved successfully" }, { status: 200 });

    } catch (error) {
        console.error("[reschedule.accept]", error);
        return NextResponse.json({ error: "Failed to accept reschedule", details: error.message }, { status: 500 });
    }
}
